 ...
 */
class Switch {
  __SWITCH_STATE__: boolean
  fun_event_on: SwitchFunEvent | null
...
class CustomAppController extends mvc.Controller {
  ...
  handle_canvas_mouse_move(event) {
    ...
    const cursor_line_left = this.view.get(EL.div_cursor_line_left).get_jq_element()
    const cursor_line_right = this.view.get(EL.div_cursor_line_right).get_jq_element()
    const cursor_line_top = this.view.get(EL.div_cursor_line_top).get_jq_element()
    const cursor_line_bottom = this.view.get(EL.div_cursor_line_bottom).get_jq_element()
  ...
  get_current_score_compute_from_error() {
    ...
    const min_y = np.list_min(y)
    const max_y = np.list_max(y)
  ...
  get_current_max_score_compute_from_smallest_error() {
    const y = this.model.get('y')
    const scaling_value = this.model.get('scaling_value')
    const error = this.model.get('smallest_error')
    const min_y = np.list_min(y)
    const max_y = np.list_max(y)
    const farthest_distance = Math.abs(max_y - min_y) * scaling_value
    const scaled_error = error / farthest_distance * 100
  ...
  initialise_plot_with_best_box(reinitialise = false) {
    ...
    const y = this.model.get('y')
    const mean = this.model.get('mean')
    const std = this.model.get('std')
    const margin_x = this.model.get('margin_x')
    const margin_y = this.model.get('margin_y')
  ...
  move_horizontal_line_cursors_on_canvas() {
    const canvas_jq = this.view.get(EL.canvas_plot).get_jq_element()
    const cursor_line_left_jq = this.view.get(EL.div_cursor_line_left).get_jq_element()
    const cursor_line_right_jq = this.view.get(EL.div_cursor_line_right).get_jq_element()
  ...
  send_ajax_and_run(url, ajax_data, success_function) {
    ...
    const inner_success_function = function (response) {
  ...
  update_canvas_plot() {
    ...
    const x = this.model.get('x')
    const new_point_x = this.model.get('new_point_x')
    const y = this.model.get('y')
    const mean = this.model.get('mean')
    const std = this.model.get('std')
    const std_low = std.slice(0, std.length / 2)
    const std_high = std.slice(std.length / 2, std.length)

    const x_data = this.model.get('x_data')
    const y_data = this.model.get('y_data')
    ...
    const is_ucb_to_display = this.model.get('settings')['display_uncertainty']
    ...
    if (bs.is_not_null_or_undefined(this.model.get('settings')['show_lookahead']) &&
      ...
      const look_ahead_query_index = this.model.get('look_ahead_query_index')
      const upboosted_points = this.model.get('upboosted_points')
      const downboosted_points = this.model.get('downboosted_points')
      console.log('upboosted_points:', upboosted_points)
      console.log('downboosted_points:', downboosted_points)
      const y_max = np.list_max(y) + 0.8
      const y_min = np.list_min(y) - 0.8

      // Makes a list of size n of elements of value val
      const f_list = function (n, val) {
        const new_array = []
        for (let i = 0; i < n; i++) { new_array.push(val) }
        return new_array
      }

      const mean_lookahead = this.model.get('look_ahead_gp_mean')
    ...
    if (this.model.get('settings')['session_score_as_max'] === true) {
      ...
      if (bs.len(y_data_actual) > 0) {
        ...
        const max_y_data_actual = y_data_actual[max_y_data_actual_index]
        const x_where_max_y_data_actual = x_data_plus_new[max_y_data_actual_index]
    ...
    // Get a scaling value
    const scaling_value = plt.get_canvas_dict().get_current_vertical_scaling_value_to_visual_space()
  ...
  update_canvas_elements() {
    ...
    const margin_around_point = 5 // px
    const canvas_jq = this.view.get(EL.canvas_plot).get_jq_element()
    const div_canvas_container = this.view.get(EL.div_canvas_container).get_jq_element()
    const cursor_line_left_jq = this.view.get(EL.div_cursor_line_left).get_jq_element()
    const cursor_line_right_jq = this.view.get(EL.div_cursor_line_right).get_jq_element()
    const cursor_line_top_jq = this.view.get(EL.div_cursor_line_top).get_jq_element()
    const cursor_line_bottom_jq = this.view.get(EL.div_cursor_line_bottom).get_jq_element()
    const max_indicator_jq = this.view.get(EL.div_max_indicator_container).get_jq_element()
    const limit_square = plt.get_canvas_dict().limit_square
    const usable_width = canvas_jq.width() // TODO: may not work if proportional is enabled, check plt.cx for details
  ...
  update_canvas_size() {
    ...
    const canvas_plot = this.view.get(EL.canvas_plot)
    const left_canvas_offset = canvas_plot.get_dom_element().offsetLeft
  ...
  send_ajax_gp_sample_request(input_dialogue_value = null) {
    ...
    const keep_current_study = this.model.get('update_session')
    ...
    const update_sampling = (this.model.get('session', false) !== undefined && keep_current_study === true)
  ...
  send_user_click_for_x_prediction(event, click_time) {
    ...
    const mouse_as_plot_coordinates = plt.get_values_from_pixels_to_coordinates(new plt.Point([mouse_x, mouse_y]))
    ...
    const data = this.model.data_dict
...
class Application extends mvc.SimpleApplication {
  constructor() {
    ...
    const controller = new CustomAppController(model, view)
  ...
  add_elements() {
    ...
    const modal = new interface_elements.ModalDialogue(EL.modal_dialogue,
      EL.button_ok_modal,
