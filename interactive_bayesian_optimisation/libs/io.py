...
def get_file_item_max(file_dir, min_val=0):
    ...
    file_id_list = [int(session_id.split(".")[0])
                    for session_id in os.listdir(file_dir)] + [min_val - 1]
    return int(np.max(file_id_list))


def ensure_savedir_exists(save_dir=None, study_name=None, sub_path=None):
    if sub_path is None:
        os.makedirs(config.get_study_save_dir(
            save_dir, study_name), exist_ok=True)
    else:
        os.makedirs(os.path.join(config.get_study_save_dir(
            save_dir, study_name), sub_path), exist_ok=True)
...
def get_new_session_id(user_id=None, save_dir=None, study_name=None):
    study_dir = os.path.join(config.get_study_save_dir(
        save_dir, study_name), str(user_id))
...
def save_data(data, user_id, session_id, save_dir=None, study_name=None, incremental=False, override_name=None):
    ...
    if incremental == False:
        ...
        if os.path.exists(session_file_path):
            save_data_list = load_data(
                user_id, session_id, study_name=study_name)
        else:
            save_data_list = list()
    ...
    # Save JSON in a folder of JSON files (one per iteration)
        ...
        ensure_savedir_exists(save_dir, study_name, os.path.join(
...
def load_settings(settings_file_name):
    ...
    if os.path.exists(settings_file_path):
        ...
        logging.warning("Settings file {} was not found in {}.".format(
