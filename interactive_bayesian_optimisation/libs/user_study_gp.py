...
from interactive_bayesian_optimisation.libs.bayesian_optimisation import upper_confidence_bound
from interactive_bayesian_optimisation.libs import gaussian_processes as gp


def data_gp_initialisation(start: float,
                           stop: float,
                           num: int,
                           kernel_name: str,
                           kernel_args: Dict[str, Any],
                           noise: float = 0.0) -> Tuple[
    List[float], List[float], int, List[float], List[float], List[float]]:
    ...
    std_upper: List[float] = (mean_vector + 2 * np.sqrt(np.diagonal(covariance_matrix + noise *
                                                                    np.identity(mean_vector.shape[0])))).tolist()
    std_lower: List[float] = (mean_vector - 2 * np.sqrt(np.diagonal(covariance_matrix + noise *
                                                                    np.identity(mean_vector.shape[0])))).tolist()
...
def look_ahead_initialisation(x: List[float],
                              kernel_name: str = None,
                              kernel_args: Dict[str, Any] = None,
                              noise: float = 0.0) -> Tuple[List[float], List[float], List[float], List[float]]:
    ...
    """
    x = np.array(x)
    kernel: Callable = kernel_functions.rbf_kernel if kernel_name is None else getattr(kernel_functions, kernel_name)
    ...
    # mean_vector, covariance_matrix = gp.zero_mean_initialise_scikit(x, kernel_args, noise=0.0)
    std_list: List[float] = (np.sqrt(np.diagonal(covariance_matrix + noise *
                                                 np.identity(mean_vector.shape[0])))).tolist()
    mean_list: List[float] = list(mean_vector.tolist())
    return (mean_list,
            std_list,
...
def update(x: Union[List[float], np.ndarray],
           kernel_name: str,
           kernel_args: Dict[str, Any],
           x_data: Union[List[float], np.ndarray],
           y_data: Union[List[float], np.ndarray],
           noise: float = 0.0,
           full_max_list: bool = False) -> Tuple[Union[int, np.ndarray], List[float], List[float], List[float]]:
    ...
    kernel_fun: Callable[[np.ndarray, np.ndarray], np.ndarray] = lambda x_, x_prime: kernel(x_, x_prime, **kernel_args)
    mean_vector, covariance_matrix = gp.regression_update(x, kernel_fun, x_data, y_data, noise)
    ...
    if full_max_list is True:
        max_selection_indices = upper_confidence_bound(mean_vector, covariance_matrix, full_max_list)
    else:
